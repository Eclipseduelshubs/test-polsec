...
local function startAutoSteal()
    ...
    Conns.autoSteal = RunService.Heartbeat:Connect(LPH_NO_VIRTUALIZE(function()
...
local function enableStretch()
    ...
    stretchConn = RunService.RenderStepped:Connect(LPH_NO_VIRTUALIZE(function()
    ...
    stretchFovConn = RunService.RenderStepped:Connect(LPH_NO_VIRTUALIZE(function()
...
local function startEnemySpeed()
    ...
    enemySpeedConn = RunService.Heartbeat:Connect(LPH_NO_VIRTUALIZE(function()
...
-- ============================================================
steppedConn = RunService.Stepped:Connect(LPH_NO_VIRTUALIZE(function()
...
function startAutoLeft()
    ...
    alConn = RunService.Heartbeat:Connect(function()
        ...
        -- 🔥 MODIFICA: usa velocità lagger se attivo
        local spd = laggerToggled and (laggerLevel == 2 and LAGGER_SPEED_2 or LAGGER_SPEED_1) or NS
...
local function startAntiRagdoll()
    ...
    antiRagdollConn = RunService.Heartbeat:Connect(LPH_NO_VIRTUALIZE(function()
...
-- ====== INFINITE JUMP (56 para modo Tap, 54 para Hold) ======
...
startAimbotAdapt = LPH_NO_VIRTUALIZE(function()
    ...
    _aimbotConn = RunService.RenderStepped:Connect(LPH_NO_VIRTUALIZE(function()
...
local function refreshSpeedModeLabel()
    if modeValLbl then
        if laggerToggled then
            modeValLbl.Text = laggerLevel == 1 and "Lagger Mode" or "Lagger Mode"
...
local function loadAllSettings()
    ...
    if data.dropMode then
        ...
        if dropModeBtnRef then
            dropModeBtnRef.Text = dropMode == 1 and "Stand" or "Jump"
        end
    end
...
local function resetToDefaults()
    ...
    if autoTPHeightBox then autoTPHeightBox.Text = tostring(autoTPDownThreshold) end 
   if batSpeedBox then batSpeedBox.Text = tostring(BAT_AIMBOT_SPEED) end
    if bypassSpeedBox then bypassSpeedBox.Text = tostring(BYPASS_AIMBOT_SPEED) end
    ...
    if modeSelectBtn then
        modeSelectBtn.Text = jumpMode == 1 and "Tap" or "Hold"
    end
...
local function buildGui()
    ...
    main.Size=UDim2.new(0,450,0,470)
    main.Position=UDim2.new(0.5, -210, 0.5, -235)  -- AL 
    ...
    sidebarImg.Image = "rbxassetid://125595227066065"
    ...
    brandFrame.Position = UDim2.new(0, 20, 0, 25)
    ...
    brandLine.Size = UDim2.new(0, 150, 0, 5)
    ...
    miniBtn.Size=UDim2.new(0,180,0,55)  -- più grande
    miniBtn.Position=UDim2.new(0, 35, 0.5, -140)  -- più sopra
    miniBtn.BackgroundColor3=BLACK
    miniBtn.BorderSizePixel=0
    miniBtn.Text="Espo Hub"
    miniBtn.TextColor3=ACCENT
    miniBtn.Font=Enum.Font.GothamBold
    miniBtn.TextSize=18  -- testo più grande
    miniBtn.ZIndex=20
    ...
    applyShimmerToText(miniBtn, 0.8)
...
pageHolder.Position = UDim2.new(0, 5, 0, -4)   -- o -6 / -8 per ancora più su
    ...
    local btnMain   = makeTopTab("SPEED / COMBAT", 1, mainPage)
local btnOther  = makeTopTab("STEAL / VISUAL", 2, otherPage)
local btnConfig = makeTopTab("MECHANICS",         3, configPage)
local btnKeybinds = makeTopTab("CONFIG/KEYBINDS",     4, keybindsPage)
    local allTabs = {
...
do local row=mkRow(40); mkLabel(row,"Lagger Carry Speed"); laggerBox=mkBox(row,LAGGER_SPEED_1,50,56,function(v) if v>0 and v<=500 then LAGGER_SPEED_1=v end end) end
do local row=mkRow(40); mkLabel(row,"Lagger Mode Speed"); lagger2Box=mkBox(row,LAGGER_SPEED_2,50,56,function(v) if v>0 and v<=500 then LAGGER_SPEED_2=v end end) end
...
mkSect("Combat")
autoBatSetVisual = mkToggle("Auto Bat", function(on) if on then enableAutoBat() else disableAutoBat() end if mobSetAutoBat then mobSetAutoBat(on) end end)
...
bypassSetVisual = mkToggle("Bypass Aimbot", function(on) toggleBypass(on) end)
...
do local row = mkRow(40); mkLabel(row, "Bypass Mode"); bypassModeBtnRef = mkSelector(row, bypassMode == 1 and "Bypass" or "TP Bat", function(btn) toggleBypassMode() btn.Text = bypassMode == 1 and "Bypass" or "TP Bat" end) end

setBatCounterVisual = mkToggle("Bat Counter", function(on) batCounterEnabled = on; if on then startBatCounter() else stopBatCounter() end end)
setMedusaVisual = mkToggle("Medusa Counter", function(on) setMedusaCounterState(on) end)
setMedusaAutoResetVisual = mkToggle("Medusa Auto Reset", function(on) setMedusaAutoResetState(on) end)
...
mkSect("Visual")
local stretchToggleSetter = mkToggle("Stretch", function(on)
...
do
    ...
    local function makeFOVBtn(val, x)
        ...
        btn.TextSize = 12
        Instance.new("UICorner", btn).CornerRadius = UDim.new(0, 5)
        local stroke = Instance.new("UIStroke", btn); stroke.Color = Color3.fromRGB(80,80,80); stroke.Thickness = 1
...
local setJumpVisual = mkToggle("Infinite Jump", function(on)
    jumpEnabled = on
    if on then startJumpMode() else stopJumpMode() end
end)

do
    local row = mkRow(40)
    mkLabel(row, "Jump Mode")
    modeSelectBtn = mkSelector(row, jumpMode == 1 and "Tap" or "Hold", function(btn)
        jumpMode = jumpMode == 1 and 2 or 1
        btn.Text = jumpMode == 1 and "Tap" or "Hold"
        if jumpEnabled then
            stopJumpMode()
            startJumpMode()
        end
...
dropBrainrotSetVisual = mkToggle("Drop Brainrot", function(on)
    if on then executeDropWithToggle(dropBrainrotSetVisual) else stopDropBrainrot() end
end)

do
    local row = mkRow(40)
    mkLabel(row, "Drop Mode")
    dropModeBtnRef = mkSelector(row, dropMode == 1 and "Stand" or "Jump", function(btn)
        dropMode = dropMode == 1 and 2 or 1
        btn.Text = dropMode == 1 and "Stand" or "Jump"
    end)
end

local setAutoTPDownVisual = mkToggle("Auto TP Down", function(on)
    autoTPDownEnabled = on
    if on then startAutoTPDown() else stopAutoTPDown() end
end)

-- === BOX ALTEZZA AUTO TP DOWN ===
do
    local row = mkRow(40)
    mkLabel(row, "Auto TP Down Height")
    autoTPHeightBox = mkBox(row, autoTPDownThreshold, 50, 56, function(v)
        if v >= 5 and v <= 100 then
            autoTPDownThreshold = v
        end
    end)
end

local setUnwalkVisual = mkToggle("Unwalk", function(on)
    unwalkEnabled = on
...
do
    ...
    fillRegion.Size = UDim2.new(0, 290, 1, -8)  
...
local ROWS = 4
    ...
    mobSetCarry = createButton("Carry", "CARRY\nSPD", 5, true, function(setActive, _)
        if not speedMode then
            ...
            if buttons.LaggerCarry and buttons.LaggerCarry.setActive then buttons.LaggerCarry.setActive(false) end
            if buttons.LaggerMode and buttons.LaggerMode.setActive then buttons.LaggerMode.setActive(false) end
        else
            speedMode=false; setActive(false)
        end
        refreshSpeedModeLabel()
    end)
    mobSetLaggerCarry = createButton("LaggerCarry", "LAGGER\nCARRY", 6, true, function(setActive, _)
...
local function createInstaResetFloatingButton()
    ...
    btnFrame.Size = UDim2.new(0, 140, 0, 60)  -- più lungo
    btnFrame.Name = "Frame"
    
    -- Posizionato sotto il mini button Espo Hub
    btnFrame.Position = UDim2.new(0, 35, 0.5, -80)  -- sotto il mini btn
    ...
    -- [Il resto del codice di dragging rimane invariato]
    local dragging = false
            ...
            if dragging then
                if not hasMoved then
                    ...
                    task.delay(0.3, function()
                        if btnFrame and btnFrame.Parent then setActive(false) end
                    end)
...
local function createBypassFloatingButton()
    ...
    -- Posizionato sotto il mini button (un po' più in basso del reset)
    btnFrame.Position = UDim2.new(0, 35, 0.5, -10)
...
steppedConn = RunService.Stepped:Connect(LPH_NO_VIRTUALIZE(function()
    ...
    movementLoop = RunService.RenderStepped:Connect(LPH_NO_VIRTUALIZE(function()
    local char = LP.Character
    if not char then return end
    local hum = char:FindFirstChildOfClass("Humanoid")
    local hrp = char:FindFirstChild("HumanoidRootPart")
    ...
    if not autoBatEnabled and not bypassToggled and not autoLeftEnabled and not autoRightEnabled then
        local md = hum.MoveDirection
        local spd = laggerToggled and (laggerLevel == 2 and LAGGER_SPEED_2 or LAGGER_SPEED_1) or (speedMode and CS or NS)
        if md.Magnitude > 0 then
            lastMoveDir = md
            hrp.Velocity = Vector3.new(md.X * spd, hrp.Velocity.Y, md.Z * spd)
        elseif antiRagdollEnabled and lastMoveDir.Magnitude > 0 then
